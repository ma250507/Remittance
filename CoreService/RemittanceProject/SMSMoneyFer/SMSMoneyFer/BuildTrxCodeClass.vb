Public Class BuildTrxCodeClass
    Public Shared Function GetRandomPIN(ByRef pPIN As String, ByVal pPINLength As Integer) As Integer
        Dim r As Double
        Dim m As Long
        Dim mv As Double
        Dim mvPIN As Long

        If pPINLength < 4 Then pPINLength = 4
        If pPINLength > 7 Then pPINLength = 7
        Try
            Randomize()
            r = Rnd()
            m = 10 ^ pPINLength
            mv = r * m
            mvPIN = Int(mv)
            pPIN = mvPIN.ToString("0000")
            Return 0
        Catch ex As Exception
            log.loglog("GetRandomPin, ex:" & ex.ToString, False)
            Return 9
        End Try
    End Function
    Public Shared Sub LockSecond(ByVal LockThread As System.Threading.Thread)
        SyncLock LockThread
